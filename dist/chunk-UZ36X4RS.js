...
function trimBase3(baseUrl) {
  return baseUrl.replace(/\/$/, "");
}
function joinBaseURL2(baseURL, endpoint) {
  const trimmed = endpoint.trim();
  if (!trimmed) return "";
  if (/^https?:\/\//i.test(trimmed)) return trimmed;
  return `${trimBase3(baseURL)}/${trimmed.replace(/^\//, "")}`;
}
function resolveBoardEndpoint(baseUrl, configured, fallback) {
  const direct = (configured ?? "").trim();
  if (direct) return joinBaseURL2(baseUrl, direct);
  return joinBaseURL2(baseUrl, fallback);
}
...
async function fetchPostForUpdate(cfg, session, boardId, postId) {
  const baseUrl = cfg.base_url?.trim() ?? "";
  if (!baseUrl) return { attaches: [] };
  const url = `${trimBase3(baseUrl)}/api/board/${boardId}/post/${postId}`;
  const out = await requestText(url, {
    method: "GET",
    headers: { Accept: "application/json, text/plain, */*" }
  }, session);
  if (out.status >= 400) return { attaches: [] };
  const parsed = parseJsonMaybe(out.text);
  return { attaches: parsed?.data?.attaches ?? [] };
}
async function boardPostCreate(cfg, session, boardId, subject, content) {
  const baseUrl = cfg.base_url?.trim() ?? "";
  if (!baseUrl) throw new Error("base url required");
...
async function boardPostUpdate(cfg, session, boardId, postId, subject, content) {
  const baseUrl = cfg.base_url?.trim() ?? "";
  if (!baseUrl) throw new Error("base url required");
  ...
  const resolvedContent = resolved.content;
  const existing = await fetchPostForUpdate(cfg, session, boardId, postId);
  const updateAttaches = existing.attaches.filter((a) => (typeof a.id === "number" || typeof a.id === "string") && typeof a.name === "string").map((a) => ({ id: String(a.id), name: String(a.name) }));
  const payload = {
    boardId: String(boardId),
    notiMailFlag: false,
    notiPushFlag: false,
...
async function boardImageUpload(cfg, session, filePath, gossoCookie) {
  const baseUrl = cfg.base_url?.trim() ?? "";
  if (!baseUrl) throw new Error("base url required");
  ...
  const file = await fs2.readFile(resolvedPath);
...
function replaceVideoTempSrcWithAttachUrl(content, boardId, postId, attaches, rewrites) {
  let out = content;
...
async function resolveInlineMedia(cfg, session, content) {
  const re = /src=(["'])\[\{([^\]\}]+)\}\]\1/g;
  ...
  for (const match of matches) {
    ...
    const parsed = parseJsonMaybe(uploadRaw);
    const data = parsed.data ?? {};
    const ext = (data.fileExt ?? "").toLowerCase();
    if (ext === "jpg" || ext === "jpeg" || ext === "png" || ext === "gif" || ext === "webp") {
      const src2 = data.hostId && data.filePath ? `/thumb/temp/${data.hostId}/original${data.filePath}` : pickUploadedSrc(data) || rawPath;
      output = output.replace(
        original,
        `data-filename=${quote}${fileName}${quote} data-filepath=${quote}${data.filePath ?? ""}${quote} data-hostid=${quote}${data.hostId ?? ""}${quote} data-inline=${quote}true${quote} src=${quote}${src2}${quote}`
      );
      continue;
    }
...
function summarizeBoardResult(raw, mode) {
  const parsed = parseJsonMaybe(raw);
  if (mode === "attach") {
    const attachId = typeof parsed.attachId === "number" || typeof parsed.attachId === "string" ? parsed.attachId : typeof parsed.id === "number" || typeof parsed.id === "string" ? parsed.id : void 0;
    if (attachId !== void 0) return `attach ok: ${String(attachId)}
`;
    return "attach ok\n";
  }
  const postId = typeof parsed.postId === "number" || typeof parsed.postId === "string" ? parsed.postId : typeof parsed.id === "number" || typeof parsed.id === "string" ? parsed.id : void 0;
  if (postId !== void 0) return `${mode} ok: ${String(postId)}
`;
  return `${mode} ok
`;
}
