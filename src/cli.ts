...
import { formatAttendanceStatus, formatConfig, formatMailOutput, formatApprovalOutput, formatLeaveCountOutput, formatCalendarOutput } from './lib/format.js';
...
import { listMail, searchMail, deleteMail, sendMail } from './lib/mail.js';
import { approvalTodo, approvalReference, approvalCount, approvalLeaveCount } from './lib/approval.js';
...
function boolFlag(flags: Map<string, string | boolean>, key: string, fallback = false): boolean {
  const v = flags.get(key);
...
async function runAttend(args: string[], attendOverride = false): Promise<number> {
...
async function runMail(args: string[]): Promise<number> {
  if (args.length === 0 || args[0] === 'help' || args[0] === '-h' || args[0] === '--help') {
  ...
  const { cfg, session } = await resolveSession();
  const baseUrl = resolveBaseUrl(cfg, session);
  const resolvedCfg = { ...cfg, base_url: baseUrl } as Config;
...
async function runCalendar(args: string[]): Promise<number> {
  if (args.length === 0 || args[0] === 'help' || args[0] === '-h' || args[0] === '--help') {
  ...
  const baseUrl = resolveBaseUrl(cfg, session);
  if (sub === 'list') {
    const calendarId = hasFlag(flags, 'calendar-id') ? flagString(flags, 'calendar-id', '') : undefined;
    const fromDate = flagString(flags, 'from-date', '');
    const toDate = flagString(flags, 'to-date', '');
    const raw = await listCalendarEvents(baseUrl, session, calendarId, fromDate || undefined, toDate || undefined);
    if (hasFlag(flags, 'json')) process.stdout.write(`${raw}\n`);
...
async function runApproval(args: string[]): Promise<number> {
  if (args.length === 0 || args[0] === 'help' || args[0] === '-h' || args[0] === '--help') {
...
async function runLeaveCount(args: string[]): Promise<number> {
  const { flags } = parseFlags(args);
...
async function resolveMailContent(flags: Map<string, string | boolean>): Promise<string> {
...
function normalizeIdFlags(args: string[]): string[] {
  const ids: string[] = [];
  for (let i = 0; i < args.length; i += 1) {
